...
btnRecherche.addEventListener('click', () => {
    ...
    //Utilise la valeur sauvegardée par le menu sur mesure 
...
// Clic dans la liste des favoris
listeFavoris.addEventListener('click', (event) => {
    
    // Cas A : L'utilisateur a cliqué sur la petite croix 
    ...
    // Cas B : L'utilisateur a cliqué n'importe où sur la ligne du favori
...
selectItems.querySelectorAll('div').forEach(item => {
    item.addEventListener('click', function() {
        ...
        //Sauvegarde le nombre choisi en mémoire globale
        window.limiteRecherche = parseInt(this.getAttribute('data-val'), 10);
        
        //Referme la liste déroulante
        selectItems.classList.add('select-hide');
        
        //Relance automatiquement la recherche si un mot est déjà tapé
